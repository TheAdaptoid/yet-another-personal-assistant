...
class TestInit:
    ...
    def test_stores_attributes(
        self, provider, mock_model_fetcher, mock_llm_invoker
    ) -> None:
        ...
        assert provider._llm_invoker is mock_llm_invoker
...
class TestInvokeLlmStream:
    ...
    async def test_streams_content_from_invoker(
        self, provider, mock_llm_invoker, sample_model, sample_messages
    ) -> None:
    ...
    async def test_passes_tools_to_invoker(
        self, provider, mock_llm_invoker, sample_model, sample_messages
    ) -> None:
        ...
        async for _ in provider.invoke_llm_stream(
            sample_model, sample_messages, tools=[tool]
        ):
            pass
    ...
    async def test_raises_model_invocation_error_on_failure(
        ...
        async def _fail(*_args, **_kwargs):  # pyright: ignore[reportUnusedCoroutine]
            raise api_error
            yield  # pragma: no cover

        mock_llm_invoker.stream_invoke = _fail
...
class TestInvokeLlmStatic:
    """Tests for InferenceProvider.invoke_llm_static()."""
    ...
    async def test_raises_error_on_non_llm_model(
        ...
        with pytest.raises(ModelInvocationError, match="not an LLM"):
            await provider.invoke_llm_static(model, sample_messages)

    async def test_returns_assistant_message(
        self, provider, mock_llm_invoker, sample_model, sample_messages
    ) -> None:
        expected = AssistantMessage(content="Hello", role="assistant")
        mock_llm_invoker.static_invoke = AsyncMock(return_value=expected)

        result = await provider.invoke_llm_static(sample_model, sample_messages)
    ...
    async def test_passes_model_id_messages_and_params(
        ...
        mock_llm_invoker.static_invoke = AsyncMock(
            return_value=AssistantMessage(content="ok", role="assistant")
        )
        params = InferenceParams(temperature=0.7, max_tokens=100, top_p=0.9)

        await provider.invoke_llm_static(sample_model, sample_messages, params=params)

        mock_llm_invoker.static_invoke.assert_awaited_once_with(
    ...
    async def test_passes_tools_to_invoker(
        ...
        await provider.invoke_llm_static(
            sample_model, sample_messages, tools=[tool]
        )
    ...
    async def test_raises_model_invocation_error_on_failure(
        ...
        mock_llm_invoker.static_invoke = AsyncMock(side_effect=api_error)

        with pytest.raises(ModelInvocationError) as exc_info:
            await provider.invoke_llm_static(sample_model, sample_messages)
